...
table_name, script_type = 'gsc_stats', 'reports'
...
for report in report_dict.keys():
    ...
    if set(metrics).issubset(['crawled', 'kb_day', 'time_download']) == False:
        continue
    ...
    cursor.execute('select max(date) from gsc_stats where url_id = %s;' , [url_id])
    ...
    if len(metrics) == 1:
        ...
        cursor.execute('select * from gsc_stats where url_id = %s and date > %s order by date asc;' , (url_id, start_date))
...
if file_emailtext != '':
    title = title_to_email_section('Google Search Console Statistics','report')
