...
table_name, script_type = 'sem_rush_dashboard', 'reports'
reports = email_to_send['sem_rush']
if reports == None:
...
for report in report_dict.keys():
    ...
    cursor.execute('select max(date) from sem_rush_dashboard;')
    ...
    if len(metrics) == 1:
        ...
        for url_id in url_ids:
            ...
            cursor.execute('select * from sem_rush_dashboard where url_id = %s and date > %s order by date asc;' , (url_id,start_date))
...
if file_emailtext != '':
    title = title_to_email_section('SEM Rush','report')
