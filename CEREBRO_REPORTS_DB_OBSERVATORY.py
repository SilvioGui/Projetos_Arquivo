...
table_name, script_type = 'observatory', 'reports'
reports = email_to_send['observatory']
if reports == None:
...
for report in report_dict.keys():
    ...
    if len(url_ids) == 1:
        ...
        for metric in metrics:
            ...
            cursor.execute('select * from observatory where url_id = %s and datetime > %s order by datetime asc;' , (url_id,start_date))
            results = cursor.fetchall()
            siteurl = results[0]['url']

            if len(results) < 5:
                file_emailtext += dict_texts['text1'][language] %(dict_colunas['semantic'][metric], siteurl, siteurl)
                logging.info('There is still not enough information to follow %s on %s' %(dict_colunas['semantic'][metric], siteurl))
            ...
            metric = dict_colunas['semantic'][metric]
...
if file_emailtext != '':
    title = title_to_email_section('Observatory','report')
