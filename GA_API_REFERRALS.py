...
argparser.add_argument('query_filter', type=str,  help=('Filter for the API query'))
...
def main(argv):
    ...
    try:
        ...
        results = get_api_query(service, flags.table_id, flags.end_date, flags.end_date, 'sessions', flags.query_filter, 'sessions', 5).execute()
        ...
        for query_dim in query_dims:      
            #Total Traffic
            results = get_api_query(service, flags.table_id, flags.start_date, flags.end_date, flags.query_metric, flags.query_filter + ';ga:source==' + query_dim, 'date', 100).execute()
            print(flags.query_filter + ';ga:source==' + query_dim)
            print_rows(results, flags.query_metric, False)    
...
def get_api_query(service, table_id, startdate, enddate, metric, channel, sort_by, n_results):
