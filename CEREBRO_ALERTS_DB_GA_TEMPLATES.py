...
table_name, script_type = 'ga_page_templates', 'alerts'
url_ids = get_url_ids('ga','Google Analytics Templates',dict)
...
for url_id in url_ids:
    if config_dict[url_id]['templates'] == [] or config_dict[url_id]['metrics'] == []:
        logging.info('Templates not checked for url_id %s' %url_id)
        continue

    colunas = config_dict[url_id]['metrics']
    traffic_types = config_dict[url_id]['templates']
    ...
    cursor.execute("select weekday(max(date)) from ga_page_templates where url_id = %s;" %url_id)
    ...
    cursor.execute("select url from ga_page_templates where url_id = %s order by date desc limit 1;", [url_id])
    results = cursor.fetchall()
    if len(results) == 0:
        logging.info('{}: there are no rows in ga_page_templates table for url_id = {}'.format(__file__, url_id))
        continue
    ...
    for traffic_type in traffic_types:
        ...
        for coluna in colunas:
            if aggregation == 'weeks':
                ...
                cursor.execute("set @today = '%s',@url_id = '%s', @template = '%s';" %(today, url_id, traffic_type))
                if coluna in lastdays:
                    if coluna == 'bounceRate' or coluna == 'avgSessionDuration':
                        sql = "select 1 as week, sum(" + coluna + "*sessions)/sum(sessions) as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 0 day) and date>=date_sub(@today, interval 7 day) and (url_id, template_name) = (@url_id, @template) union select 2 as week, sum(" + coluna + "*sessions)/sum(sessions) as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 7 day) and date>=date_sub(@today, interval 14 day) and (url_id, template_name) = (@url_id, @template) union select 3 as week, sum(" + coluna + "*sessions)/sum(sessions) as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 14 day) and date>=date_sub(@today, interval 21 day) and (url_id, template_name) = (@url_id, @template) union select 4 as week, sum(" + coluna + "*sessions)/sum(sessions) as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 21 day) and date>=date_sub(@today, interval 28 day) and (url_id, template_name) = (@url_id, @template) union select 5 as week, sum(" + coluna + "*sessions)/sum(sessions) as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 28 day) and date>=date_sub(@today, interval 35 day) and (url_id, template_name) = (@url_id, @template) union select 6 as week, sum(" + coluna + "*sessions)/sum(sessions) as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 35 day) and date>=date_sub(@today, interval 42 day) and (url_id, template_name) = (@url_id, @template) union select 7 as week, sum(" + coluna + "*sessions)/sum(sessions) as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 42 day) and date>=date_sub(@today, interval 49 day) and (url_id, template_name) = (@url_id, @template) union select 8 as week, sum(" + coluna + "*sessions)/sum(sessions) as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 49 day) and date>=date_sub(@today, interval 56 day) and (url_id, template_name) = (@url_id, @template) union select 9 as week, sum(" + coluna + "*sessions)/sum(sessions) as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 56 day) and date>=date_sub(@today, interval 63 day) and (url_id, template_name) = (@url_id, @template) union select 10 as week, sum(" + coluna + "*sessions)/sum(sessions) as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 63 day) and date>=date_sub(@today, interval 70 day) and (url_id, template_name) = (@url_id, @template);"
                    elif coluna == 'avgTimeOnPage':
                        sql = "select 1 as week, sum(" + coluna + "*pageviews)/sum(pageviews) as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 0 day) and date>=date_sub(@today, interval 7 day) and (url_id, template_name) = (@url_id, @template) union select 2 as week, sum(" + coluna + "*pageviews)/sum(pageviews) as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 7 day) and date>=date_sub(@today, interval 14 day) and (url_id, template_name) = (@url_id, @template) union select 3 as week, sum(" + coluna + "*pageviews)/sum(pageviews) as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 14 day) and date>=date_sub(@today, interval 21 day) and (url_id, template_name) = (@url_id, @template) union select 4 as week, sum(" + coluna + "*pageviews)/sum(pageviews) as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 21 day) and date>=date_sub(@today, interval 28 day) and (url_id, template_name) = (@url_id, @template) union select 5 as week, sum(" + coluna + "*pageviews)/sum(pageviews) as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 28 day) and date>=date_sub(@today, interval 35 day) and (url_id, template_name) = (@url_id, @template) union select 6 as week, sum(" + coluna + "*pageviews)/sum(pageviews) as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 35 day) and date>=date_sub(@today, interval 42 day) and (url_id, template_name) = (@url_id, @template) union select 7 as week, sum(" + coluna + "*pageviews)/sum(pageviews) as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 42 day) and date>=date_sub(@today, interval 49 day) and (url_id, template_name) = (@url_id, @template) union select 8 as week, sum(" + coluna + "*pageviews)/sum(pageviews) as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 49 day) and date>=date_sub(@today, interval 56 day) and (url_id, template_name) = (@url_id, @template) union select 9 as week, sum(" + coluna + "*pageviews)/sum(pageviews) as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 56 day) and date>=date_sub(@today, interval 63 day) and (url_id, template_name) = (@url_id, @template) union select 10 as week, sum(" + coluna + "*pageviews)/sum(pageviews) as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 63 day) and date>=date_sub(@today, interval 70 day) and (url_id, template_name) = (@url_id, @template);"
                    else:
                        sql = "select 1 as week, avg(" + coluna + ") as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 0 day) and date>=date_sub(@today, interval 7 day) and (url_id, template_name) = (@url_id, @template) union select 2 as week, avg(" + coluna + ") as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 7 day) and date>=date_sub(@today, interval 14 day) and (url_id, template_name) = (@url_id, @template) union select 3 as week, avg(" + coluna + ") as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 14 day) and date>=date_sub(@today, interval 21 day) and (url_id, template_name) = (@url_id, @template) union select 4 as week, avg(" + coluna + ") as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 21 day) and date>=date_sub(@today, interval 28 day) and (url_id, template_name) = (@url_id, @template) union select 5 as week, avg(" + coluna + ") as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 28 day) and date>=date_sub(@today, interval 35 day) and (url_id, template_name) = (@url_id, @template) union select 6 as week, avg(" + coluna + ") as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 35 day) and date>=date_sub(@today, interval 42 day) and (url_id, template_name) = (@url_id, @template) union select 7 as week, avg(" + coluna + ") as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 42 day) and date>=date_sub(@today, interval 49 day) and (url_id, template_name) = (@url_id, @template) union select 8 as week, avg(" + coluna + ") as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 49 day) and date>=date_sub(@today, interval 56 day) and (url_id, template_name) = (@url_id, @template) union select 9 as week, avg(" + coluna + ") as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 56 day) and date>=date_sub(@today, interval 63 day) and (url_id, template_name) = (@url_id, @template) union select 10 as week, avg(" + coluna + ") as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 63 day) and date>=date_sub(@today, interval 70 day) and (url_id, template_name) = (@url_id, @template);"
                else:
                    sql = "select 1 as week, sum(" + coluna + ") as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 0 day) and date>=date_sub(@today, interval 7 day) and (url_id, template_name) = (@url_id, @template) union select 2 as week, sum(" + coluna + ") as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 7 day) and date>=date_sub(@today, interval 14 day) and (url_id, template_name) = (@url_id, @template) union select 3 as week, sum(" + coluna + ") as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 14 day) and date>=date_sub(@today, interval 21 day) and (url_id, template_name) = (@url_id, @template) union select 4 as week, sum(" + coluna + ") as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 21 day) and date>=date_sub(@today, interval 28 day) and (url_id, template_name) = (@url_id, @template) union select 5 as week, sum(" + coluna + ") as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 28 day) and date>=date_sub(@today, interval 35 day) and (url_id, template_name) = (@url_id, @template) union select 6 as week, sum(" + coluna + ") as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 35 day) and date>=date_sub(@today, interval 42 day) and (url_id, template_name) = (@url_id, @template) union select 7 as week, sum(" + coluna + ") as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 42 day) and date>=date_sub(@today, interval 49 day) and (url_id, template_name) = (@url_id, @template) union select 8 as week, sum(" + coluna + ") as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 49 day) and date>=date_sub(@today, interval 56 day) and (url_id, template_name) = (@url_id, @template) union select 9 as week, sum(" + coluna + ") as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 56 day) and date>=date_sub(@today, interval 63 day) and (url_id, template_name) = (@url_id, @template) union select 10 as week, sum(" + coluna + ") as " + coluna + ", min(date) as initial_date, max(date) as date from ga_page_templates where date<date_sub(@today, interval 63 day) and date>=date_sub(@today, interval 70 day) and (url_id, template_name) = (@url_id, @template);"
                analysis = 'weeks'
                ...
                if coluna in lastweekdays:
                    ...
                    sql = "select * from ga_page_templates where (weekday(date),url_id,template_name)=('%s', '%s', '%s') order by date desc limit 0,10;" %(str(maxdate), url_id, traffic_type)
                    analysis = 'weekdays'
                elif coluna in lastdays:
                    #gets last 10 day entries for the specified site, 9 for statistic analysis and 1 to be checked
                    sql = "select * from ga_page_templates where (url_id,template_name)=('%s', '%s') order by date desc limit 0,11;" %(url_id, traffic_type)
                    analysis = 'days'
                else:
            ...
            if len(var) >= 5:
                ...
                logging.info("No data available for %s %s. Proceding to next template..." %(siteurl, traffic_type))
                break
            ...
            url_break = False
            cursor.execute("select ga_page_templates from alerts_analysis_history where url_id = '%s';" %url_id)
            last_alert_date = cursor.fetchall()
            if last_alert_date != ():
                last_alert_date = last_alert_date[0]['ga_page_templates']         #gets the last date from the alerts sent in past
                    ...
                    try:                                                #tries to access the last date for the current email by dictionary key, if doesnt exist, moves on
                        ...
                        if last_analyzed_date == str(date1):            #in case there is a last date for the current email, checks if it is the same as the date being analyzed
                            ...
                            if sent_date != ():
                                ...
                                logging.info("There is already an email alert sent to %s about Google Analytics Page Templates data collected for %s in %s. \
...
A new alert on the same issue won't be sent." %(toaddr, siteurl, date1))
    ...
    if url_trends != [] or url_outliers != []:
        ...
        url_outliers, url_prior_outliers, outlier_additional_text = drop_similar_dim_alerts(url_outliers, url_prior_outliers, 'outlier', 'templates', 3, traffic_types, str)
        url_trends, url_prior_trends, trends_additional_text = drop_similar_dim_alerts(url_trends, url_prior_trends, 'trends', 'templates', 3, traffic_types, str)
...
#email sending part
if file_emailtext != '':
    title = title_to_email_section('Google Analytics Page Templates','unusual')
