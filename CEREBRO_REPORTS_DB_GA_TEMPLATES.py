...
table_name, script_type = 'ga_page_templates', 'reports'
...
for report in report_dict.keys():
    ...
    cursor.execute('select max(date) from ga_page_templates where url_id = %s;' , [url_id])
    ...
    if len(metrics) == 1:
        ...
        for dimension in dimensions:            
            cursor.execute('select * from ga_page_templates where (url_id, template_name) = (%s, %s) and date > %s order by date asc;' , (url_id, dimension, start_date))
...
if file_emailtext != '':
    title = title_to_email_section('Google Analytics Templates','report')
