...
def plot_n_curves(x, y, x_label_dates, n, names):
    ...
    for y_set, curve_name in zip(y, names):
        ...
        y_smooth=f(x_sm)
        curve_name, dimension_condition = get_semantic_dimention([metric, curve_name, '', False])
        report_values[m] = curve_name
...
table_name, script_type = 'ga_channels', 'reports'
...
for report in report_dict.keys():
    if 'channels' not in report_dict[report]:
        continue
    dimensions = report_dict[report]['channels']
    if type(dimensions[0]) != list:
        logging.info('The alert is not set properly. The dimensions should be a list with the Device in first position and Channel in the Second')
        continue
    ...
    cursor.execute('select max(date) from ga_channels where url_id = %s;' , [url_id])
    ...
    if len(metrics) == 1:
        ...
        for dimension in dimensions:            
            cursor.execute('select * from ga_channels where (url_id, device, channel) = (%s, %s, %s) and date > %s order by date asc;' , (url_id, dimension[0], dimension[1], start_date))
            results = cursor.fetchall()
            if results == ():
                logging.info('There isn\'t any information to follow %s on url id: %s %s' %(dimension[0], dimension[1], url_id))
            ...
            if len(results) < 5:
                ...
                logging.info('There is still not enough information to follow %s on %s %s' %(dimension[0], dimension[1], siteurl))
...
if file_emailtext != '':
    title = title_to_email_section('Google Analytics Channels','report')
