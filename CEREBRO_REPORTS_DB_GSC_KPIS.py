...
table_name, script_type = 'gsc_kpis', 'reports'
...
for report in report_dict.keys():
    if 'kpis' not in report_dict[report]:
        continue
    dimensions = report_dict[report]['kpis']
    if type(dimensions[0]) != list:
        logging.info('The alert is not set properly. The dimensions should be a list with the Device in first position and Dimension in the Second')
        continue
    ...
    cursor.execute('select max(date) from gsc_kpis where url_id = %s;' , [url_id])
    ...
    if len(metrics) == 1:
        ...
        for dimension in dimensions:            
            cursor.execute('select * from gsc_kpis where (url_id, device, type) = (%s, %s, %s) and date > %s order by date asc;' , (url_id, dimension[0], dimension[1], start_date))
...
if file_emailtext != '':
    title = title_to_email_section('Google Search Console KPIs','report')
