...
table_name, script_type = 'alexa', 'reports'
reports = email_to_send['alexa']
if reports == None:
...
for report in report_dict.keys():
    ...
    cursor.execute('select max(date) from alexa;')
    ...
    if len(metrics) == 1:
        ...
        for url_id in url_ids:
            ...
            cursor.execute('select * from alexa where url_id = %s and date > %s order by date asc;' , (url_id,start_date))
...
if file_emailtext != '':
    title = title_to_email_section('Alexa','report')
